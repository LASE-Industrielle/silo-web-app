  ...
  return (
    ...
    <div style={style.container}>
      <div style={style.readText}>
        {unreadCount === 0 ? (
          <div>{t("Read notifications")} ({readCount})</div>
        ) : (
          <div>{t('unreadNotifications', {unreadCount})}</div>
        )}
      </div>
      ...
      <div style={style.footer}>
        <div style={style.allRead}>{t("Mark all as read")}</div>
        <div style={style.clearAll}>{t("Clear all")}</div>
      </div>
    </div>
  )
}
